...
class QdrantRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(
        ...
        *,
        ...
        # Fallback: broad queries can yield 0 hits when score_threshold is high.
        # Retry with a relaxed threshold so we always provide some catalog context.
        if not hits and self.score_threshold > 0:
            hits = search_chunks(
                query_vector=query_vec,
        ...
        # For parent-child: use chunk_text as page_content (richer context for LLM)
        ...
        for hit in hits:
            ...
            # Deduplicate by chunk_id so the LLM doesn't see the same
            ...
            chunk_content = chunk_text if chunk_text else hit["chunk_text"]
            docs.append(
                Document(
...
class RAGService:
    ...
    def generate(self, query: str) -> Dict[str, Any]:
        ...
        product_url = list(
            {doc.metadata.get("product_url", "") for doc in evidence if doc.metadata.get("product_url")}
        )
