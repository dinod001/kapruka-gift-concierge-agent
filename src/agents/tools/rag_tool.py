...
# When running this file directly (e.g. `python src/agents/tools/rag_tool.py`),
# ensure `src/` is importable so `services.*` / `infrastructure.*` work.
...
class RAGTool:
    ...
    def _raw_search(
        ...
        for idx, hit in enumerate(results, 1):
            sim = f"{hit['score']:.2f}"
            product_name = hit.get("product_name") or "Unknown product"
            url = hit.get("product_url") or "N/A"
            text = hit.get("chunk_text", "")
            lines.append(f"\n--- Chunk {idx} (similarity {sim}) ---")
            lines.append(f"Source: {product_name} ({url})")
            lines.append(text)
...
if __name__ == "__main__":
    query = (
        "My dad loves dark chocolate but doesn't like anything too sweet. Can you find a premium chocolate gift pack available on Kapruka that fits his taste?"
    )
    tool = RAGTool(embedder=get_default_embeddings(), llm=get_chat_llm())
